        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    conn.reply(m.chat, `❤🥖 ¡Se agotaron tus *🥖 Baguettes*! Teto necesita más pan francés para seguir cantando.`, m, rcanal)
                    continue
                }
                let extra = {
                    match,
                ...
                try {
                    ...
                    if (m.limit)
                        conn.reply(m.chat, `❤ Utilizaste *${+m.limit}* 🥖 Baguetts en esta operación.`, m, rcanal)
                }
                break
            }
        }
